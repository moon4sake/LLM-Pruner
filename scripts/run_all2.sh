...
sparsity_values=("0.25" "0.50" "0.75" "0.10")
...
run_pipeline() {
    ...
    echo y | CUDA_VISIBLE_DEVICES=${gpu_id} python post_training.py --prune_model prune_log/${prune_ckpt_path}/pytorch_model.bin \
        ...
        --wandb_project DistillPrune --lora_r 8 --num_epochs 3 \
    ...
    # echo y | CUDA_VISIBLE_DEVICES=${gpu_id} bash scripts/evaluate.sh ${base_model} tune_log/${name}_s${sparsity}_block prune_log/${name}_s${sparsity}_block 2000
