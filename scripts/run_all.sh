...
run_pipeline() {
    ...
    if ! (echo y | CUDA_VISIBLE_DEVICES=${gpu_id} python llama3.py --base_model ${base_model} \
        ...
        --block_mlp_layer_start 4 --block_mlp_layer_end 26 --block_attention_layer_start 4 \
        --block_attention_layer_end 26 --save_ckpt_log_name ${prune_ckpt_path} --pruner_type taylor \
    ...
    echo y | CUDA_VISIBLE_DEVICES=${gpu_id} bash scripts/evaluate.sh ${base_model} ${sparsity} tune_log/${name}_s${sparsity}_block prune_log/${name}_s${sparsity}_block 1800
