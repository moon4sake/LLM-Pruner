...
sparsity_values=("0.00" "0.25" "0.50")
...
run_pipeline() {
    ...
    if [ "$sparsity" == "0.00" ]; then
        ...
        echo y | CUDA_VISIBLE_DEVICES=${gpu_id} bash scripts/evaluate_pro.sh ${base_model} ${sparsity} tune_log/${tune_ckpt_path} prune_log/${prune_ckpt_path} 2400
    fi
