...
run_pipeline() {
    ...
    if [ "$sparsity" == "0.00" ]; then
        echo y | CUDA_VISIBLE_DEVICES=${gpu_id} bash scripts/evaluate.sh "math" ${base_model} ${sparsity} "" prune_log/${name}_s${sparsity}_block 0
    else
        echo y | CUDA_VISIBLE_DEVICES=${gpu_id} bash scripts/evaluate.sh "math" ${base_model} ${sparsity} tune_log/${tune_ckpt_path} prune_log/${prune_ckpt_path} 5000
    fi
