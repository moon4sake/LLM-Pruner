...
def main():
    ...
    def tokenize(prompt, add_eos_token=True):
        result = tokenizer(
            prompt,
            truncation=True,
            max_length=training_args.cutoff_len,
            padding="max_length",
            return_tensors=None,
        )
        if (
            result["input_ids"][-1] != tokenizer.eos_token_id
            and len(result["input_ids"]) < training_args.cutoff_len
            and add_eos_token
        ):
    ...
    if model_args.model_name_or_path.endswith(".bin"):  # Binary model
        model = torch.load(
            model_args.model_name_or_path, map_location=model_kwargs["device_map"], weights_only=False
        )
    ...
    if training_args.do_eval:
        ...
        metrics["eval_samples"] = len(val_data)
