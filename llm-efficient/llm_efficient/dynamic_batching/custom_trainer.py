...
from alignment import decontaminate_humaneval, get_datasets, apply_chat_template
...
class CustomSFTTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.each_domain_eval_datasets = each_domain_eval_datasets
        self.n_domains = len(each_domain_eval_datasets)
        self.proportion = proportion
        self.target_loss = target_loss

        eval_packing = False  # self.args.packing if self.args.eval_packing is None else self.args.eval_packing
        self.formatting_func = get_formatting_func_from_dataset(
            self.train_dataset, self.tokenizer
        )

        for idx, each_dataset in enumerate(self.each_domain_eval_datasets):

            self.each_domain_eval_datasets[idx] = ConstantLengthDataset(
                self.tokenizer,
                self.each_domain_eval_datasets[idx]["test"],
                dataset_text_field=(
                    None
                    if self.formatting_func is not None
                    else self.args.dataset_text_field
                ),
                formatting_func=self.formatting_func,
                seq_length=self.args.max_seq_length,
                infinite=False,
                num_of_sequences=self.args.num_of_sequences,
                chars_per_token=self.args.chars_per_token,
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            ########################################
            if epoch >= 1:
                train_dataloader = self.update_trainloader()
            ...
            ### Change Dataset
            ########################################################################################
            eval_losses = self.calculate_eval_loss(ignore_keys_for_eval)
            self.get_new_dataset_based_on_loss(eval_losses)
            ########################################################################################
    ...
    def calculate_eval_loss(self, ignore_keys_for_eval):

        loss_values = []

        for eval_dataset in self.each_domain_eval_datasets:

            eval_metrics = self.evaluate(
                eval_dataset,
                ignore_keys=ignore_keys_for_eval,
                metric_key_prefix="multi_batch",
            )
            loss_values.append(eval_metrics["multi_batch_loss"])

        return loss_values

    ##########################################################################################

    def update_proportion(self, losses):

        current_prop = deepcopy(self.proportion)
        ...
        self.proportion = updated_domain_weights.tolist()

    ##########################################################################################

    def update_train_dataset(self):

        data_list = self.data_args.dataset_mixer.keys()

        for dataname, probability in zip(data_list, self.proportion):
            self.data_args.dataset_mixer[dataname] = probability

        raw_datasets = get_datasets(
            self.data_args,
            splits=self.data_args.dataset_splits,
            configs=self.data_args.dataset_configs,
            columns_to_keep=[
                "messages",
                "chosen",
                "rejected",
        ...
        train_dataset = self._prepare_dataset(
            ...
            remove_unused_columns=(
                self.args.remove_unused_columns if self.args is not None else True
            ),
    ...
    def get_new_dataset_based_on_loss(self, eval_losses):
        self.update_proportion(eval_losses)
