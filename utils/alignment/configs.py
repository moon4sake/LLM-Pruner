...
class SFTConfig(trl.SFTConfig):
    ...
    cutoff_len: int = field(
        default=256,
        metadata={"help": "Length to cut off input sequences"},
    )
    ...
    use_reentrant: bool = field(
        default=True,
        metadata={"help": "Whether to use reentrant mode for gradient checkpointing"},
    )
    val_set_size: int = field(
        default=50,
        metadata={"help": "Size of the validation set"},
    )
    train_on_inputs: bool = field(
        default=False,
        metadata={"help": "Train on inputs. If False, masks out inputs in loss"},
    )
    add_eos_token: bool = field(
        default=False,
        metadata={"help": "Whether to add EOS token"},
    )
